...
pub enum LocalStorageVec<T, const N: usize> {
    ...
    Heap(Vec<T>), // to make the compiler happy
}
...
impl<T, const N: usize> Debug for LocalStorageVec<T, N>
where
    T: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Stack { buf, len } => f
                .debug_struct("Stack")
                .field("buf", buf)
                .field("len", len)
                .finish(),
            Self::Heap(v) => f.debug_struct("Heap").field("v", v).finish(),
        }
    }
}

impl<T: Default + Copy + Display, const N: usize> LocalStorageVec<T, N> {
    pub fn new() -> Self {
    ...
    pub fn into_iter(&self) -> Iter<'_, T> {
        match self {
            Self::Stack { buf, len: _ } => buf.into_iter(),
            Self::Heap(v) => v.into_iter(),
        }
    }
}
